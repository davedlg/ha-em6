...
class em6CarbonSensor(Entity):
    def __init__(self, name, api):
        ...
        self._unit_of_measurement = 'g/kWh'
        self._unique_id = DOMAIN + "_carbon"
        self._api = api
...
class em6RenewablesSensor(Entity):
    def __init__(self, name, api):
        self._name = name
        self._icon = "mdi:chart-bar"
        self._state = None
        ...
        self._unique_id = DOMAIN  + "_renewables"
        self._api = api
    ...
    async def async_update(self):
        ...
        if response:
            ...
            # Avoid updating the data (state) if the price is still the same or we will get duplicate notifications
            if self._state != response['nz_renewable']:
                self._state = response['nz_renewable']
                self._state_attributes['Last Updated'] = response['timestamp']
        else:
            self._state = None
            _LOGGER.warning('Found no renewable data on refresh')
...
class em6EnergyPriceSensor(Entity):
    def __init__(self, name, api):
        ...
        self._state_class = "measurement"
        self._unit_of_measurement = '$'
        self._unique_id = DOMAIN  + "_energyprice"
        self._api = api
